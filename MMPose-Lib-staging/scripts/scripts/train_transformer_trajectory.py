...
# Load TrajectoryDataset from existing script to share normalization behavior
...
class TransformerModel(nn.Module):
    def __init__(self, d_model: int = 128, nhead: int = 8, num_layers: int = 3,
                 dim_feedforward: int = 512, dropout: float = 0.1, pred_len: int = 8):
        super().__init__()
...
def evaluate(model, loader, device, dataset):
    ...
    for inp, tgt in loader:
        ...
        # unnormalize predicted and target deltas back to scaled units
        ...
        # reconstruct positions from last input
...
def main():
    ...
    parser.add_argument("--save", type=str, default="transformer_traj.pt")
    ...
    model = TransformerModel(d_model=args.d_model, nhead=args.nhead, num_layers=args.num_layers,
                             dim_feedforward=args.ff_dim, dropout=args.dropout, pred_len=PRED_LEN).to(device)
