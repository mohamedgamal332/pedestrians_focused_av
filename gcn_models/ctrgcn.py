...
class EnhancedCTRGCN(nn.Module):
    ...
    def _load_pretrained_weights(self, path):
        ...
        # Project pretrained weights onto custom graph (ignore adjacency buffers)
        filtered_dict = {}
        for k, v in pretrained_dict.items():
            if k in model_dict and 'A_init' not in k and model_dict[k].shape == v.shape:
                filtered_dict[k] = v
            elif k in model_dict and 'A_init' not in k:
